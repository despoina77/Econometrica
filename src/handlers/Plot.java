...
public class Plot extends JFrame{
    ...
    public Plot(final String title, CountryDataset oil, CountryDataset gdp) {    
        //Τίτλος του Διαγράμματος
        super(title);
        // Δημιουργία τίτλου διαγράμματος
        this.oil = oil;
        this.gdp = gdp;
    ...
    //Δημιουργια των δεδομενων GDP του πρωτου αξονα, και επιστροφη των τιμων.
    ...
    //Δημιουργια των δεδομενων OIL του δευτερου αξονα, και επιστροφη των τιμων.
    private XYDataset createDataset2() {
        final TimeSeries s2 = new TimeSeries("Oil Data ", Year.class);

        if(this.oil != null){
            for(CountryData oildata: this.oil.getCountryDataCollection()){
                CountryData cd = new CountryData();
                s2.add(new Year(Integer.parseInt(oildata.getDataYear())), Double.parseDouble(oildata.getValue()));
            }
        }
        final TimeSeriesCollection dataset = new TimeSeriesCollection();
